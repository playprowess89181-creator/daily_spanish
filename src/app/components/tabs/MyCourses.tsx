...
export default function MyCourses() {
  ...
  const totals = useMemo(() => {
    ...
    const videoLessons = lessons.filter((l) => !!l.video_type).length;
    const pdfLessons = lessons.filter((l) => !!(l.has_lesson_pdf || l.has_keys_pdf)).length;
    return { totalLessons, videoLessons, pdfLessons };
  }, [lessons]);
  ...
  const openLesson = (l: Lesson) => {
    const url = `/dashboard/lessons/${encodeURIComponent(l.id)}?block=${encodeURIComponent(activeBlock)}`;
    router.push(url);
  };
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-6">
        <div className="flex items-center justify-between">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="rounded-lg border border-neutral-200 bg-neutral-50 px-4 py-2 text-center">
              <div className="text-xl font-bold text-neutral-900">{totals.videoLessons}</div>
              <div className="text-xs text-neutral-600">Videos</div>
            </div>
            <div className="rounded-lg border border-neutral-200 bg-neutral-50 px-4 py-2 text-center">
              <div className="text-xl font-bold text-neutral-900">{totals.pdfLessons}</div>
              <div className="text-xs text-neutral-600">PDFs</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-5 gap-4">
          {blocks.map((block) => {
            ...
            const withPdf = (byBlock[block.id] || []).filter((l) => l.has_lesson_pdf || l.has_keys_pdf).length;
            return (
              <div key={block.id} className="rounded-xl border border-neutral-200 bg-neutral-50 p-4">
                ...
                <div className="mt-2 text-xs text-neutral-600">{count} lessons · {withPdf} with PDFs</div>
              </div>
            );
          })}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
          {(() => {
            ...
            return arr.map((l, i) => {
              ...
              const hasAnyPdf = !!(l.has_lesson_pdf || l.has_keys_pdf);
              return (
                <div
                  key={l.id}
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="mt-2 flex items-center justify-center gap-1 text-[10px]">
                      <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded border ${l.video_type ? 'bg-white/80 border-white/60' : 'bg-white/40 border-white/30'}`}>
                        <i className={`fas ${l.video_type === 'link' ? 'fa-link' : 'fa-video'}`}></i>
                        Video
                      </span>
                      <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded border ${hasAnyPdf ? 'bg-white/80 border-white/60' : 'bg-white/40 border-white/30'}`}>

...
type ExerciseSetDetail = {
  id: number;
  title: string;
  entries: Array<{ id: number; word: string; image_name: string; created_at: string }>;
  created_at: string;
};
...
function EditVocabularyExercisePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      <div className="lg:ml-64 flex flex-col flex-1">
        <Header title="Edit Vocabulary Exercise" onToggleSidebar={() => setSidebarOpen(true)} />
            ...
            {!loading && !error && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="glass-effect rounded-xl p-6 border border-white/20">
                  <label className="block text-sm font-medium text-gray-700 mb-2">Replace Words via CSV</label>
                  <div
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          const images = new Set<string>(Array.isArray((imagesData as any)?.images) ? (imagesData as any).images : []);
                ...
                <div className="space-y-6">
                  <div className="bg-white rounded-lg shadow-md p-6">
                    <h4 className="text-md font-semibold text-gray-900 mb-3">Current Words</h4>
                    {!detail || detail.entries.length === 0 ? (
                      <p className="text-sm text-gray-700">No words in this exercise.</p>
                    ) : (
                      <ul className="space-y-2">
                        {detail.entries.slice(0, 200).map((e) => (
                          <li key={e.id} className="flex justify-between text-sm text-gray-700">
                            <span>{e.word}</span>
                            <span className="text-gray-500">{e.image_name}</span>
                          </li>
                        ))}
                      </ul>
                    )}
                    {detail && detail.entries.length > 200 && <p className="mt-3 text-xs text-gray-500">Showing first 200 rows.</p>}
                  </div>
...
export default withAdminAuth(EditVocabularyExercisePage);

...
export default function LoginPage() {
  ...
  const [statusPopup, setStatusPopup] = useState<{ title: string; message: string } | null>(null);
  const { login } = useAuth();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.code === 'blocked' || result.code === 'inactive') {
          setStatusPopup({
            title: result.code === 'blocked' ? 'Account Blocked' : 'Account Deactivated',
            message:
              result.error ||
              (result.code === 'blocked'
                ? 'Your account has been blocked. Please contact support to regain access.'
  ...
  return (
              ...
              {currentForm === 'login' && (
                <form onSubmit={handleLogin} className="space-y-5">
                  <div>
                    ...
                    <input
                      autoComplete="email"
                      className="w-full px-4 py-3 rounded-xl outline-none border-2 border-gray-200 text-gray-900 placeholder:text-gray-500 focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-all duration-200"
                      style={{
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full px-4 py-3 pr-12 rounded-xl outline-none border-2 border-gray-200 text-gray-900 placeholder:text-gray-500 focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-all duration-200"
                        style={{
                          background: 'rgba(255, 255, 255, 0.8)',
                          backdropFilter: 'blur(2px)'
                        }}
                    ...
                    <div className="flex justify-center space-x-3 mb-4">
                      {otp.map((digit, index) => (
                        <input
                          ...
                          className="w-12 h-12 text-center text-xl font-bold text-gray-900 rounded-xl border-2 border-gray-200 focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-all duration-200"
                          style={{
                            background: 'rgba(255, 255, 255, 0.8)',
                            backdropFilter: 'blur(2px)'
                          }}
    ...
    {statusPopup && (
      <div className="fixed inset-0 z-[200] flex items-center justify-center px-4">
        <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => setStatusPopup(null)}></div>
        <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-200 flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="text-lg font-extrabold text-gray-900 tracking-tight">{statusPopup.title}</div>
              <div className="mt-1 text-sm font-semibold text-gray-600">Access restricted</div>
            </div>
            <button
              type="button"
              className="h-9 w-9 inline-flex items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 hover:bg-gray-50 focus:outline-none"
          ...
          <div className="px-5 py-4 text-sm font-semibold text-gray-700 leading-relaxed">{statusPopup.message}</div>
